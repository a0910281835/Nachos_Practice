...
PendingInterrupt::PendingInterrupt(CallBackObj *callOnInt, int time, IntType kind)
{
    ...
    //cout << "pending Interrupt : " << intTypeNames[kind] << endl;
}
...
bool Interrupt::CheckIfDue(bool advanceClock)
{
    ...
    //next = pending->Front();
    //cout << "this have ? " << intTypeNames[next->type] << " at time " << next->when << endl;
    inHandler = FALSE;
    return TRUE;
}
